...
date = 2025-08-04T07:07:07+01:00
draft = false
+++

It is completely naive to deny that LLMs and foundational models 
have changed the course of the Deep learning research forever. 
I think one can draw a line which divides the research papers and novel contributions 
before the first LLM and after it. 
...
The first time I read about deep learning was by the 
end of 2014, it was merely an accident and curiosity that 
drove my consistent persistence to learn about neural networks. 

At that time it was not yet a sexy field 
that everyone wanted to consider for future career prospectives at least for 
CS 4th year students who were all into web and mobile development. 
There was a large rise in the need for software engineers 
at that time and data science was not yet something. 

After a couple of years, a noticeable trend was that
Deep Learning became the de-facto
standard for doing research in computer vision. Before it was all about the combination of filters, 
feature extraction and Support Vector Machines which 
were mostly coded in MatLab. Later on it became all about 
Convolution Neural Networks. Same also applies for NLP, where tasks as machine translation, sentiment analysis,
text summarization, and recommendation was solved primarily by recurrent neural networks and LSTMs.


Deep Learning has done the 
disturbance in the research and publishing in fields as 
Computer Vision way before LLMs had done to the whole ML field, and other engineering fields were AI now became 
the secret keyword to get access to grants. 

At that time one would always get that sarcastic comments as: “DL is just statistics in disguise”. 
My naive answer would be that it just works. 
The idea of pouring large scale and consistently curated datasets 
from any domain into a neural networks with millions/billions of
parameters and training for hundreds of hours on GPUs seems to
be an inevitable success. 
It is theoretically sound because of the Universal approximation theorem, and ML researchers proved that it really works practically.


I never have doubts until I had to 
do my master thesis project which was a bit of different course than the popular 
research or models at that time. 
I worked on solving combinatorial optimization 
problems with neural networks. At the end when I achieved my goal, 
I realized that I did two things that were really important to the 
success of the models that I have built. 
First is the inductive bias that I have introduced and 
the second is scale. 
The inductive bias I have introduced 
was a simple attention mechanism based on cosine similarity [^1], 
so I would assume that in general it could be transferable to other problems too. 

The important lesson I learned is that when you 
deal with a new problem, it is really important to understand the domain of the problem and don’t just throw data to a neural network. Nevertheless, scale is the key. 
